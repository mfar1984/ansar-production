exports.id=6579,exports.ids=[6579],exports.modules={28303:a=>{function b(a){var b=Error("Cannot find module '"+a+"'");throw b.code="MODULE_NOT_FOUND",b}b.keys=()=>[],b.resolve=b,b.id=28303,a.exports=b},35552:(a,b,c)=>{"use strict";c.d(b,{P:()=>e});let d=c(29382).createPool({host:process.env.DB_HOST||"localhost",user:process.env.DB_USER||"root",password:process.env.DB_PASSWORD||"root",database:process.env.DB_NAME||"ansar",waitForConnections:!0,connectionLimit:10,queueLimit:0});async function e(a,b){let[c]=b&&b.length>0?await d.query(a,b):await d.query(a);return c}},36657:(a,b,c)=>{"use strict";c.d(b,{TC:()=>k,lN:()=>h,KB:()=>l,DI:()=>j,S5:()=>g});var d=c(35552);function e(a){return String(a||"").split(/\r?\n/).map(a=>a.trim()).filter(Boolean)}let f={isOpen:!1,heading:"Shop",intro:"",perPage:12,howToOrder:"",hideSoldOut:!1,showStockLeft:!0};async function g(){try{let a=(await (0,d.P)(`SELECT is_open, page_heading, intro_text, products_per_page, how_to_order,
              hide_sold_out, show_stock_left
         FROM market_place_settings WHERE id = 1`))[0];if(!a)return f;let b=Number(a.products_per_page);return{isOpen:1===Number(a.is_open),heading:(a.page_heading||"").trim()||f.heading,intro:(a.intro_text||"").trim(),perPage:Number.isFinite(b)?Math.min(48,Math.max(4,Math.trunc(b))):f.perPage,howToOrder:(a.how_to_order||"").trim(),hideSoldOut:1===Number(a.hide_sold_out),showStockLeft:1===Number(a.show_stock_left)}}catch{return f}}async function h(){try{let a=await (0,d.P)("SELECT is_open FROM market_place_settings WHERE id = 1");return 1===Number(a[0]?.is_open)}catch{return!1}}function i(a){return 1===Number(a.track_stock)&&(Number(a.option_count)>0?null!==a.option_stock&&0>=Number(a.option_stock):0>=Number(a.stock_quantity))}async function j(a){try{let b=await (0,d.P)(`SELECT p.id, p.name, p.slug, p.short_description, p.price, p.compare_at_price,
