"use strict";(()=>{var a={};a.id=3684,a.ids=[3684],a.modules={3498:a=>{a.exports=require("mysql2/promise")},3557:(a,b,c)=>{c.d(b,{$3:()=>g,J9:()=>j,O4:()=>l,OC:()=>f,OD:()=>h,QU:()=>m,Sj:()=>k,ah:()=>n,oS:()=>i});var d=c(88251),e=c(63415);async function f(a,b){let c=await (0,e.iT)(a);return c||(b.status(401).json({success:!1,error:"Your session is invalid or has expired. Please sign in again."}),null)}function g(a,b,c){return(0,e.$3)(a,b,c)}function h(a,b,c,d){return!!g(a,c,d)||(b.status(403).json({success:!1,error:`You do not have permission for this: ${c}_${d}`}),!1)}function i(a,b){let c=[],d=[];for(let[e,f]of Object.entries(b))e in a&&(c.push(`\`${e}\` = ?`),d.push(function(a,b){if(void 0===b)return null;if("bool"===a)return+(!0===b||1===b||"1"===b||"true"===b);if("number"===a){if(null===b||""===b)return null;let a=Number(b);return Number.isFinite(a)?a:null}if("json"===a){if(null===b||""===b)return null;if("string"==typeof b)return b;try{return JSON.stringify(b)}catch{return null}}if(null===b)return null;let c=String(b);return""===c?null:c}(f,a[e])));return 0===c.length?null:{clause:c.join(", "),values:d}}function j(a){let b=a.body;if(!b)return{};if("string"==typeof b)try{return JSON.parse(b)}catch{return{}}return"object"==typeof b?b:{}}async function k(){let a=await (0,d.P)("SELECT * FROM system_settings WHERE id = 1");return a.length>0?a[0]:(await (0,d.P)("INSERT INTO system_settings (id) VALUES (1)"),(await (0,d.P)("SELECT * FROM system_settings WHERE id = 1"))[0]||{})}async function l(){let a=await (0,d.P)("SELECT * FROM integrations WHERE id = 1");return a.length>0?a[0]:(await (0,d.P)("INSERT INTO integrations (id) VALUES (1)"),(await (0,d.P)("SELECT * FROM integrations WHERE id = 1"))[0]||{})}function m(a,b){let c={...a};for(let a of b){let b=c[a];c[`${a}_set`]="string"==typeof b&&b.length>0,c[a]=""}return c}function n(a,b){let c={...a};for(let a of b)a in c&&(""===c[a]||null===c[a]||void 0===c[a])&&delete c[a];return c}},8493:(a,b,c)=>{c.r(b),c.d(b,{config:()=>C,default:()=>B,handler:()=>E});var d={};c.r(d),c.d(d,{default:()=>y});var e=c(29046),f=c(8667),g=c(33480),h=c(86435),i=c(88251),j=c(3557),k=c(19275),l=c(34461),m=c(95514);let n=a=>null==a?"":String(a),o=a=>{let b="number"==typeof a?a:parseFloat(String(a??""));return Number.isFinite(b)?b:0},p=a=>{if(a instanceof Date){if(Number.isNaN(a.getTime()))return null;let b=a=>String(a).padStart(2,"0");return`${a.getFullYear()}-${b(a.getMonth()+1)}-${b(a.getDate())}`}let b=n(a).trim();return b?b.slice(0,10):null},q=(...a)=>{for(let b of a){let a=n(b).trim();if(a)return a}return""},r=(...a)=>q(...a)||null;async function s(a,b){let c=await (0,i.P)(`SELECT a.*, c.title AS job_title, c.department
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,v)).insertId,x=0;try{let a=new Date().getFullYear(),b=await (0,i.P)("SELECT id, days_per_year FROM leave_types WHERE status = ? AND days_per_year > 0",["active"]);for(let c of b)await (0,i.P)(`INSERT INTO leave_balances (
          employee_id, leave_type_id, year, total_days, used_days, carried_forward
        ) VALUES (?, ?, ?, ?, 0, 0)
        ON DUPLICATE KEY UPDATE total_days = VALUES(total_days)`,[w,c.id,a,c.days_per_year]);x=b.length}catch(a){console.error("Leave balance initialization error:",a)}return{created:!0,employeeId:t,id:w,leaveTypes:x}}var t=c(96543),u=c(80283),v=c(91823);let w={pending:"offered",approved:"hired",rejected:"rejected"},x="applicants";async function y(a,b){let c=await (0,j.OC)(a,b);if(!c)return;b.setHeader("Cache-Control","no-store");let d=Number(a.query.id);if(!Number.isInteger(d)||d<1)return b.status(400).json({success:!1,error:"Invalid applicant id."});try{if("GET"===a.method){if(!(0,j.OD)(c,b,x,"view"))return;let a=await (0,i.P)(`SELECT a.*,
           ...
           FROM career_applicants a
           LEFT JOIN career_postings c ON c.id = a.career_posting_id
          WHERE a.id = ? LIMIT 1`,[d]);if(0===a.length)return b.status(404).json({success:!1,error:"Applicant not found."});let e=await (0,l.gf)(x);return b.status(200).json({success:!0,applicant:a[0],chain:e})}let e=await (0,i.P)(`SELECT a.id, a.application_no, a.full_name, a.email, a.status, a.current_level,
         ...
         FROM career_applicants a
         LEFT JOIN career_postings c ON c.id = a.career_posting_id
        WHERE a.id = ? LIMIT 1`,[d]);if(0===e.length)return b.status(404).json({success:!1,error:"Applicant not found."});let f=e[0];if("DELETE"===a.method){if(!(0,j.OD)(c,b,x,"delete"))return;if("archived"===f.status)return b.status(409).json({success:!1,error:`${f.full_name} is already archived.`});return await (0,i.P)(`UPDATE career_applicants
            SET status = 'archived', status_before_archive = ?, archived_at = NOW(),
                reviewed_at = NOW(), reviewed_by = ?
          WHERE id = ?`,[f.status,c.adminId,d]),await (0,k.At)(a,{action:"UPDATE",module:"Career: Applicants",target:`Applicant: ${f.application_no}`,description:`${f.full_name} archived from ${u.$j[f.status]}`,before:{status:f.status},after:{status:"archived",status_before_archive:f.status}}),b.status(200).json({success:!0,message:`${f.full_name} archived. Restore puts them back at ${u.$j[f.status]}.`})}if("PATCH"!==a.method)return b.setHeader("Allow","GET, PATCH, DELETE"),b.status(405).json({success:!1,error:"Method not allowed"});let g=(0,j.J9)(a),h=void 0===g.status||null===g.status||""===g.status?null:g.status;if(null!==h&&!(0,u.F3)(h))return b.status(400).json({success:!1,error:`"${String(h)}" is not an applicant status.`});if("archived"===h)return b.status(400).json({success:!1,error:"Archiving is not a status change — send DELETE, which also records the stage being left so Restore can put it back."});if(h&&h!==f.status){let a=(0,u.aJ)(f.status,h);if(a)return b.status(409).json({success:!1,error:a})}if("hired"===h||"rejected"===h&&"offered"===f.status){if(!(0,j.OD)(c,b,x,"approve"))return;let e="hired"===h?"approve":"reject",k=(0,t.gx)(g.remarks,500)||(0,t.gx)(g.admin_notes,500)||null,m=await (0,l.jt)(x,f.current_level||0,c.adminId,c.isSuperAdmin);if(m)return b.status(403).json({success:!1,error:m});let n=await (0,l.ex)({module:x,applicationId:d,action:e,actor:{adminId:c.adminId,name:c.username},remarks:k,currentStatus:f.status,currentLevel:f.current_level||0,statusWords:w});await (0,i.P)("UPDATE career_applicants SET reviewed_at = NOW(), reviewed_by = ? WHERE id = ?",[c.adminId,d]),"reject"===e&&await (0,i.P)("UPDATE career_applicants SET rejection_reason = ? WHERE id = ?",[(0,t.gx)(g.rejection_reason,65535)||k,d]);let o="";if(n.finalized&&"approved"===n.status)try{let a=await s(d,c.adminId);o=a.created?` Employee record ${a.employeeId} created with ${a.leaveTypes} leave balance(s).`:"already-exists"===a.reason?" An employee record already existed for this person, so none was created.":" The applicant record could not be re-read, so no employee was created."}catch(a){console.error("Employee creation error:",a),o=" The employee record could NOT be created: "+(a instanceof Error?a.message:"unknown error")+". Create it from Employee Management."}let p=f.application_no||`#${d}`;await (0,l.aV)(a,{module:x,reference:p,outcome:n,beforeStatus:f.status,beforeLevel:f.current_level||0,remarks:k});let q=await (0,l.OL)({module:x,applicationId:d,reference:p,employee:f.email?{email:f.email,name:f.full_name}:null,outcome:n,actorName:c.username,remarks:k}),r=await (0,l.gf)(x),u="rejected"===n.status?`${f.full_name} was not hired.`:n.finalized?`${f.full_name} is hired.${o}`:`Approved at level ${n.level} of ${r.length}. ${f.full_name} stays at Offer Made until the remaining level(s) sign.`;return b.status(200).json({success:!0,status:n.writtenStatus,level:n.level,total_levels:n.totalLevels,finalized:n.finalized,email_sent:q.sent,email_error:q.sent?null:q.reason,message:u})}let m=[],n=[],o=!!h&&h!==f.status;if(o){if(!(0,j.OD)(c,b,x,"approve"))return;m.push("status = ?"),n.push(h)}let p=["interview_date","interview_mode","interview_location","interview_link","interviewer","interview_duration_mins","interview_notes"].some(a=>void 0!==g[a]),q=null;if(p){if(!(0,j.OD)(c,b,x,"edit"))return;let a=h||f.status;if(!u.Dd.includes(a))return b.status(409).json({success:!1,error:`An interview cannot be arranged while the applicant is at "${u.$j[a]}". Shortlist them first — that is the record that somebody read the application before inviting them in.`});q=(0,u.bE)(g.interview_mode)?g.interview_mode:"in_person";let d=(0,t.gx)(g.interview_location,200),e=(0,t.gx)(g.interview_link,400),i=u.mm[q];if("location"===i&&!d)return b.status(400).json({success:!1,error:"An in-person interview needs a location — the applicant has to be told where to go."});if("link"===i&&!e)return b.status(400).json({success:!1,error:"An online interview needs a meeting link."});let k=(0,t.gx)(g.interview_date,40);if(void 0!==g.interview_date){if(!k)return b.status(400).json({success:!1,error:"An interview needs a date and time."});m.push("interview_date = ?"),n.push(k.replace("T"," ").slice(0,19))}m.push("interview_mode = ?"),n.push(q),m.push("interview_location = ?"),n.push(d||null),m.push("interview_link = ?"),n.push(e||null),m.push("interviewer = ?"),n.push((0,t.gx)(g.interviewer,200)||null);let l=Number(g.interview_duration_mins);m.push("interview_duration_mins = ?"),n.push(Number.isInteger(l)&&l>0&&l<=600?l:null),void 0!==g.interview_notes&&(m.push("interview_notes = ?"),n.push((0,t.gx)(g.interview_notes,65535)||null))}if(void 0!==g.rejection_reason){if(!o&&!(0,j.OD)(c,b,x,"edit"))return;m.push("rejection_reason = ?"),n.push((0,t.gx)(g.rejection_reason,65535)||null)}if(void 0!==g.admin_notes){if(!o&&!p&&!(0,j.OD)(c,b,x,"edit"))return;m.push("admin_notes = ?"),n.push((0,t.gx)(g.admin_notes,65535)||null)}if(0===m.length)return b.status(400).json({success:!1,error:"Nothing to change."});m.push("reviewed_at = NOW()"),m.push("reviewed_by = ?"),n.push(c.adminId),n.push(d),await (0,i.P)(`UPDATE career_applicants SET ${m.join(", ")} WHERE id = ?`,n);let r=[];if(o&&"interview-scheduled"===h)try{let a=(0,t.gx)(g.interview_date,40)||f.interview_date||"";await (0,v.Mn)({full_name:f.full_name,email:f.email,application_no:f.application_no,position:f.job_title||"Position",interview_date:a,interview_location:(0,t.gx)(g.interview_location,200)||void 0,interview_notes:(0,t.gx)(g.interview_notes,65535)||f.interview_notes||void 0})}catch(a){r.push(`interview invitation (${a instanceof Error?a.message:"failed"})`)}if(o&&"shortlisted"===h)try{await (0,v.jo)({full_name:f.full_name,email:f.email,application_no:f.application_no,position:f.job_title||"Position",expected_salary:Number(f.expected_salary)||0,interview_date:f.interview_date||"",interview_notes:f.interview_notes||""})}catch(a){r.push(`shortlist notice (${a instanceof Error?a.message:"failed"})`)}if(o&&"rejected"===h)try{await (0,v.Hi)({full_name:f.full_name,email:f.email,application_no:f.application_no,position:f.job_title||"Position",rejection_reason:(0,t.gx)(g.rejection_reason,65535)||f.rejection_reason||""})}catch(a){r.push(`rejection notice (${a instanceof Error?a.message:"failed"})`)}return await (0,k.At)(a,{action:"UPDATE",module:"Career: Applicants",target:`Applicant: ${f.application_no}`,description:o?`${f.full_name} moved from ${u.$j[f.status]} to ${u.$j[h]}`:p?`Interview details updated for ${f.full_name}`:`Notes updated for ${f.full_name}`,before:{status:f.status},after:{status:h||f.status,interview_mode:q}}),b.status(200).json({success:!0,status:h||f.status,message:(o?`${f.full_name} is now ${u.$j[h]}.`:p?"Interview details saved.":"Saved.")+(r.length>0?` The record is updated, but the ${r.join(" and ")} could not be sent — check the email profile under Career > Settings > Notifications.`:""),email_errors:r})}catch(a){return console.error("Applicant API error:",a),b.status(500).json({success:!1,error:a instanceof Error?a.message:"Failed to update the applicant"})}}var z=c(58112),A=c(18766);let B=(0,h.M)(d,"default"),C=(0,h.M)(d,"config"),D=new g.PagesAPIRouteModule({definition:{kind:f.A.PAGES_API,page:"/api/admin/applicants/[id]",pathname:"/api/admin/applicants/[id]",bundlePath:"",filename:""},userland:d,distDir:".next",relativeProjectDir:""});async function E(a,b,c){let d=await D.prepare(a,b,{srcPage:"/api/admin/applicants/[id]"});if(!d){b.statusCode=400,b.end("Bad Request"),null==c.waitUntil||c.waitUntil.call(c,Promise.resolve());return}let{query:f,params:g,prerenderManifest:h,routerServerContext:i}=d;try{let c=a.method||"GET",d=(0,z.getTracer)(),e=d.getActiveScopeSpan(),j=D.instrumentationOnRequestError.bind(D),k=async e=>D.render(a,b,{query:{...f,...g},params:g,allowedRevalidateHeaderKeys:[],multiZoneDraftMode:!1,trustHostHeader:!1,previewProps:h.preview,propagateError:!1,dev:D.isDev,page:"/api/admin/applicants/[id]",internalRevalidate:null==i?void 0:i.revalidate,onError:(...b)=>j(a,...b)}).finally(()=>{if(!e)return;e.setAttributes({"http.status_code":b.statusCode,"next.rsc":!1});let f=d.getRootSpanAttributes();if(!f)return;if(f.get("next.span_type")!==A.BaseServerSpan.handleRequest)return void console.warn(`Unexpected root span type '${f.get("next.span_type")}'. Please report this Next.js issue https://github.com/vercel/next.js`);let g=f.get("next.route");if(g){let a=`${c} ${g}`;e.setAttributes({"next.route":g,"http.route":g,"next.span_name":a}),e.updateName(a)}else e.updateName(`${c} ${a.url}`)});e?await k(e):await d.withPropagatedContext(a.headers,()=>d.trace(A.BaseServerSpan.handleRequest,{spanName:`${c} ${a.url}`,kind:z.SpanKind.SERVER,attributes:{"http.method":c,"http.target":a.url}},k))}catch(a){if(D.isDev)throw a;(0,e.sendError)(b,500,"Internal Server Error")}finally{null==c.waitUntil||c.waitUntil.call(c,Promise.resolve())}}},21572:a=>{a.exports=require("nodemailer")},55511:a=>{a.exports=require("crypto")},75600:a=>{a.exports=require("next/dist/compiled/next-server/pages-api.runtime.prod.js")},80283:(a,b,c)=>{c.d(b,{$j:()=>f,Co:()=>e,Dd:()=>m,F3:()=>h,aJ:()=>i,bE:()=>l,hm:()=>d,mm:()=>k});let d=["active","inactive","draft"],e=["pending","shortlisted","interview-scheduled","interview-completed","offered","hired","rejected","withdrawn","archived"],f={pending:"Pending Review",shortlisted:"Shortlisted","interview-scheduled":"Interview Scheduled","interview-completed":"Interview Completed",offered:"Offer Made",hired:"Hired",rejected:"Rejected",withdrawn:"Withdrawn",archived:"Archived"},g={pending:["shortlisted","rejected","withdrawn"],shortlisted:["interview-scheduled","offered","rejected","withdrawn"],"interview-scheduled":["interview-completed","rejected","withdrawn"],"interview-completed":["offered","shortlisted","rejected","withdrawn"],offered:["hired","rejected","withdrawn"],hired:[],rejected:[],withdrawn:[],archived:[]};function h(a){return"string"==typeof a&&e.includes(a)}function i(a,b){if(a===b||g[a].includes(b))return null;let c=g[a];return 0===c.length?`${f[a]} is final — this applicant cannot be moved again.`:`An applicant at "${f[a]}" can only move to ${c.map(a=>f[a]).join(", ")}.`}let j=["in_person","online","phone"],k={in_person:"location",online:"link",phone:null};function l(a){return"string"==typeof a&&j.includes(a)}let m=["shortlisted","interview-scheduled","interview-completed"]},91823:(a,b,c)=>{c.d(b,{BR:()=>k,Dw:()=>g,Hi:()=>j,Mn:()=>h,jo:()=>i});var d=c(2066),e=c(2922);let f={async sendMail(a){let b=await (0,e.it)("applicants"),c=b.email_profile||"hr",f=await (0,d.OT)(c,{to:a.to,subject:a.subject,html:a.html,cc:a.cc||b.cc_email||void 0});if(!f.success)throw Error(f.error||`the "${c}" email profile could not send`);return f}};async function g(a){let b={from:"hr@ansartechnologies.my",to:"hr@ansartechnologies.my",subject:`New Job Application - ${a.application_no}`,html:`
    ...
    `};try{return await f.sendMail(b),{success:!0}}catch(a){return console.error("Error sending application confirmation:",a),{success:!1,error:a}}}},96543:(a,b,c)=>{function d(a){if(null==a||""===a)return null;let b=Number(String(a).replace(/,/g,"").trim());return Number.isFinite(b)?b:null}function e(a){if(null==a||""===a)return null;let b=String(a).trim().slice(0,10);return/^\d{4}-\d{2}-\d{2}$/.test(b)?b:null}function f(a,b){if(null==a)return null;let c=String(a).trim();return""===c?null:c.slice(0,b)}function g(a){if(null==a||""===a)return null;let b=Number(a);return Number.isInteger(b)&&b>0?b:null}function h(a,b,c){let d=String(a??"").trim().toLowerCase();return b.includes(d)?d:c}function i(a){let b=a.replace(/\\/g,"\\\\").replace(/%/g,"\\%").replace(/_/g,"\\_");return`%${b}%`}c.d(b,{AR:()=>i,Bq:()=>r,Mw:()=>l,N6:()=>o,Ov:()=>x,TG:()=>d,_$:()=>e,aI:()=>w,af:()=>s,cM:()=>u,fk:()=>g,gx:()=>f,i9:()=>n,iv:()=>v,jI:()=>t,jT:()=>p,ni:()=>j,qB:()=>q,yH:()=>k,yL:()=>h,zN:()=>m});let j=["draft","in_progress","submitted","evaluation","awarded","unsuccessful","closed"],k=["government","private"],l=["open","selective","quotation","direct"],m=["technical","financial","both"],n=["pending","in_progress","completed"],o=["active","completed"],p=["unsuccessful","cancelled","expired"],q=["active","inactive","blacklisted"],r=["prospect","active","inactive"],s=["government","education","healthcare","private","local_government","other"],t=["new","contacted","qualified","proposal","negotiation","won","lost"],u=["referral","cold_call","website","event","tender","other"],v=["draft","sent","accepted","rejected","expired"];function w(a){if(!a)return"open";let b=new Date(a);if(Number.isNaN(b.getTime()))return"open";let c=new Date;c.setHours(0,0,0,0),b.setHours(0,0,0,0);let d=Math.round((b.getTime()-c.getTime())/864e5);return d<0?"closed":d<=7?"closing":"open"}function x(a){return!a||a<=0?"":a<1024?`${a} B`:a<1048576?`${Math.round(a/1024)} KB`:`${(a/1048576).toFixed(1)} MB`}}};var b=require("../../../../webpack-api-runtime.js");b.C(a);var c=b.X(0,[7169,2816,4461,5514],()=>b(b.s=8493));module.exports=c})();