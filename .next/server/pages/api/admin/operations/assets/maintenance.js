"use strict";(()=>{var a={};a.id=4643,a.ids=[4643],a.modules={3498:a=>{a.exports=require("mysql2/promise")},57381:(a,b,c)=>{c.r(b),c.d(b,{config:()=>t,default:()=>s,handler:()=>v});var d={};c.r(d),c.d(d,{default:()=>p});var e=c(29046),f=c(8667),g=c(33480),h=c(86435),i=c(88251),j=c(3557),k=c(96543),l=c(95514),m=c(92102);let n={internal:"assets_internal",external:"assets_external"},o=`a.holder = ?
            ...
            ORDER BY name ASC`)]),e=await (0,m.g)();return b.status(200).json({success:!0,assets:a,vendors:c,repair_categories:e})}let f=["a.holder = ?"],g=[d],h=async a=>(await (0,i.P)("SELECT id FROM asset_repair_categories WHERE name = ? LIMIT 1",[a])).length>0,l=(0,k.gx)(a.query.kind,60);if(l){if(!await h(l))return b.status(400).json({success:!1,error:"Unknown service kind."});f.push("m.kind = ?"),g.push(l)}let n=(0,k.gx)(a.query.state,20);"overdue"===n?f.push("m.next_due IS NOT NULL AND m.next_due < CURDATE()"):"due_30"===n?f.push("m.next_due IS NOT NULL AND m.next_due >= CURDATE() AND m.next_due <= DATE_ADD(CURDATE(), INTERVAL 30 DAY)"):"scheduled"===n?f.push("m.next_due IS NOT NULL"):"unscheduled"===n&&f.push("m.next_due IS NULL");let p=(0,k.gx)(a.query.from,10);p&&(f.push("m.service_date >= ?"),g.push(p));let q=(0,k.gx)(a.query.to,10);q&&(f.push("m.service_date <= ?"),g.push(q));let r=(0,k.gx)(a.query.q,120);if(r){f.push("(a.asset_no LIKE ? OR a.name LIKE ? OR m.summary LIKE ? OR m.performed_by LIKE ? OR v.name LIKE ?)");let a=`%${r}%`;g.push(a,a,a,a,a)}let s=(0,k.gx)(a.query.round,400),t=null;if(s){if(!(t=function(a){try{let b=JSON.parse(Buffer.from(a,"base64url").toString("utf8")),c=String(b.kind??"").trim(),d=String(b.service_date??"").trim();if(!c||c.length>60||!/^\d{4}-\d{2}-\d{2}$/.test(d))return null;let e=b.client_id;if(null==e)return{kind:c,service_date:d,client_id:null};let f=Number(e);if(!Number.isInteger(f)||f<1)return null;return{kind:c,service_date:d,client_id:f}}catch{return null}}(s)))return b.status(400).json({success:!1,error:"That round is not a valid reference."});if(!await h(t.kind))return b.status(400).json({success:!1,error:"Unknown service kind."});f.push("m.kind = ?","m.service_date = ?","a.client_id <=> ?"),g.push(t.kind,t.service_date,t.client_id)}let u=`WHERE ${f.join(" AND ")}`,v=Math.max(1,Number(a.query.page)||1),w=Math.min(100,Math.max(1,Number(a.query.per_page)||100)),x=(v-1)*w,y=async()=>{let a=await (0,i.P)(`SELECT
           ...
           ${u}`,g);return b.status(200).json({success:!0,asset_ids:a.map(a=>Number(a.asset_id))})}if("1"===String(a.query.group||"")){let a=await (0,i.P)(`SELECT m.kind,
