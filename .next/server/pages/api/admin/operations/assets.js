   ...
   WHERE ti.handover_date IS NOT NULL
     ...
     AND NOT EXISTS (SELECT 1 FROM asset_obligations od
            ...
            WHERE ${b}.asset_id = ${a}.id)`}},75600:a=>{a.exports=require("next/dist/compiled/next-server/pages-api.runtime.prod.js")},88251:(a,b,c)=>{c.d(b,{Ay:()=>i,G$:()=>h,P:()=>f,rN:()=>g});var d=c(3498);let e=c.n(d)().createPool({host:process.env.DB_HOST||"localhost",user:process.env.DB_USER||"root",password:process.env.DB_PASSWORD||"root",database:process.env.DB_NAME||"ansar",waitForConnections:!0,connectionLimit:10,queueLimit:0});async function f(a,b){let[c]=b&&b.length>0?await e.query(a,b):await e.query(a);return c}async function g(){try{return(await e.getConnection()).release(),!0}catch(a){return console.error("Database connection test failed:",a),!1}}function h(){return{totalConnections:10,activeConnections:0,idleConnections:0,queuedRequests:0}}let i=e},99558:(a,b,c)=>{c.r(b),c.d(b,{config:()=>w,default:()=>v,handler:()=>y});var d={};c.r(d),c.d(d,{default:()=>q});var e=c(29046),f=c(8667),g=c(33480),h=c(86435),i=c(88251),j=c(3557),k=c(19275),l=c(96543),m=c(92102),n=c(21674),o=c(95514);let p={internal:"assets_internal",external:"assets_external"};async function q(a,b){let c=await (0,j.OC)(a,b);if(!c)return;b.setHeader("Cache-Control","no-store");let d=String(a.query.holder||"").trim();if(!o.F7.includes(d))return b.status(400).json({success:!1,error:"A register must be named: holder=internal or holder=external."});let e=p[d];try{if("GET"===a.method){if(!(0,j.OD)(c,b,e,"view"))return;if("1"===String(a.query.options||"")){let[a,c,d,e,f,g,h,j,k,l,n]=await Promise.all([(0,i.P)(`SELECT id, name, asset_class, icon, useful_life_years, default_loanable, detail_form
              ...
              WHERE is_active = 1 ORDER BY sort_order ASC, name ASC`),(0,m.g)()]);return b.status(200).json({success:!0,categories:a,employees:c,departments:d,clients:e,projects:f,vendors:g,tenders:h,sites:j,models:k,locations:l,repair_categories:n})}let f=["a.holder = ?"],g=[d],h=String(a.query.status||"").trim();if(h&&"all"!==h){if(!o.aN.includes(h))return b.status(400).json({success:!1,error:"Unknown status filter."});f.push("a.status = ?"),g.push(h)}let k=(0,l.gx)(a.query.category,80);k&&(f.push("a.category = ?"),g.push(k));let p=(0,l.gx)(a.query.asset_class,20);if(p){if(!o.Aj.includes(p))return b.status(400).json({success:!1,error:"Unknown kind filter."});f.push("a.asset_class = ?"),g.push(p)}"1"===String(a.query.needs_label||"")&&f.push("(a.serial_no IS NULL OR a.serial_no = '') AND (a.tag_no IS NULL OR a.tag_no = '') AND a.label_printed_at IS NULL");let q="1"===String(a.query.all||"");if(q&&!(0,j.OD)(c,b,e,"export"))return;let r=Number(a.query.per_page),s=q?5e3:Number.isFinite(r)?Math.min(100,Math.max(1,Math.trunc(r))):100,t=Number(a.query.page),u=q||!Number.isFinite(t)?1:Math.max(1,Math.trunc(t)),v=(u-1)*s,w=String(a.query.q||"").trim();if(w){let a=(0,l.AR)(w);f.push("(a.asset_no LIKE ? ESCAPE '\\\\' OR a.name LIKE ? ESCAPE '\\\\' OR a.serial_no LIKE ? ESCAPE '\\\\' OR a.tag_no LIKE ? ESCAPE '\\\\' OR a.brand LIKE ? ESCAPE '\\\\' OR a.model LIKE ? ESCAPE '\\\\' OR e.full_name LIKE ? ESCAPE '\\\\' OR c.company_name LIKE ? ESCAPE '\\\\')"),g.push(a,a,a,a,a,a,a,a)}let x=await (0,i.P)(`SELECT a.id, a.asset_no, a.holder, a.name, a.category, a.brand, a.model,
          ...
          LIMIT ${s} OFFSET ${v}`,g),y=await (0,i.P)(`SELECT COUNT(*) AS n
           ...
           LEFT JOIN employees    e ON e.id = a.employee_id
           LEFT JOIN client_users c ON c.id = a.client_id
          WHERE ${f.join(" AND ")}`,g),z=Number(y[0]?.n||0),A=await (0,i.P)("SELECT status, COUNT(*) AS n FROM assets WHERE holder = ? GROUP BY status",[d]),B={};for(let a of o.aN)B[a]=0;let C=0;for(let a of A)B[a.status]=Number(a.n),C+=Number(a.n);let D=o.jS.map(()=>"?").join(", "),E=await (0,i.P)(`SELECT COALESCE(SUM(purchase_cost), 0) AS cost, COUNT(*) AS n
           FROM assets
          WHERE holder = ? AND ownership = 'ansar'
            AND status NOT IN (${D}) AND purchase_cost IS NOT NULL`,[d,...o.jS]),F=await (0,i.P)(`SELECT COUNT(*) AS n FROM assets
          WHERE holder = ? AND ownership = 'client' AND status NOT IN (${D})`,[d,...o.jS]),G=await (0,i.P)(`SELECT
         ...
         ) c`,[d,...o.jS]),H=await (0,i.P)(`SELECT COUNT(DISTINCT a.id) AS n
           FROM assets a
