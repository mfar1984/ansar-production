   ...
   WHERE ti.handover_date IS NOT NULL
     ...
     AND NOT EXISTS (SELECT 1 FROM asset_obligations od
            ...
            WHERE ${b}.asset_id = ${a}.id)`}},75600:a=>{a.exports=require("next/dist/compiled/next-server/pages-api.runtime.prod.js")},99558:(a,b,c)=>{c.r(b),c.d(b,{config:()=>w,default:()=>v,handler:()=>y});var d={};c.r(d),c.d(d,{default:()=>q});var e=c(29046),f=c(8667),g=c(33480),h=c(86435),i=c(88251),j=c(3557),k=c(19275),l=c(96543),m=c(92102),n=c(21674),o=c(95514);let p={internal:"assets_internal",external:"assets_external"};async function q(a,b){let c=await (0,j.OC)(a,b);if(!c)return;b.setHeader("Cache-Control","no-store");let d=String(a.query.holder||"").trim();if(!o.F7.includes(d))return b.status(400).json({success:!1,error:"A register must be named: holder=internal or holder=external."});let e=p[d];try{if("GET"===a.method){if(!(0,j.OD)(c,b,e,"view"))return;if("1"===String(a.query.options||"")){let[a,c,d,e,f,g,h,j,k,l]=await Promise.all([(0,i.P)(`SELECT id, name, asset_class, icon, useful_life_years, default_loanable
              ...
              WHERE is_active = 1 ORDER BY sort_order ASC, name ASC`)]);return b.status(200).json({success:!0,categories:a,employees:c,departments:d,clients:e,projects:f,vendors:g,tenders:h,sites:j,models:k,locations:l})}let f=["a.holder = ?"],g=[d],h=String(a.query.status||"").trim();if(h&&"all"!==h){if(!o.aN.includes(h))return b.status(400).json({success:!1,error:"Unknown status filter."});f.push("a.status = ?"),g.push(h)}let k=(0,l.gx)(a.query.category,80);k&&(f.push("a.category = ?"),g.push(k));let m=(0,l.gx)(a.query.asset_class,20);if(m){if(!o.Aj.includes(m))return b.status(400).json({success:!1,error:"Unknown kind filter."});f.push("a.asset_class = ?"),g.push(m)}"1"===String(a.query.needs_label||"")&&f.push("(a.serial_no IS NULL OR a.serial_no = '') AND (a.tag_no IS NULL OR a.tag_no = '') AND a.label_printed_at IS NULL");let p="1"===String(a.query.all||"");if(p&&!(0,j.OD)(c,b,e,"export"))return;let q=Number(a.query.per_page),r=p?5e3:Number.isFinite(q)?Math.min(100,Math.max(1,Math.trunc(q))):100,s=Number(a.query.page),t=p||!Number.isFinite(s)?1:Math.max(1,Math.trunc(s)),u=(t-1)*r,v=String(a.query.q||"").trim();if(v){let a=(0,l.AR)(v);f.push("(a.asset_no LIKE ? ESCAPE '\\\\' OR a.name LIKE ? ESCAPE '\\\\' OR a.serial_no LIKE ? ESCAPE '\\\\' OR a.tag_no LIKE ? ESCAPE '\\\\' OR a.brand LIKE ? ESCAPE '\\\\' OR a.model LIKE ? ESCAPE '\\\\' OR e.full_name LIKE ? ESCAPE '\\\\' OR c.company_name LIKE ? ESCAPE '\\\\')"),g.push(a,a,a,a,a,a,a,a)}let w=await (0,i.P)(`SELECT a.id, a.asset_no, a.holder, a.name, a.category, a.brand, a.model,
          ...
          WHERE ${f.join(" AND ")}
          ORDER BY a.id DESC
          LIMIT ${r} OFFSET ${u}`,g),x=await (0,i.P)(`SELECT COUNT(*) AS n
           FROM assets a
           LEFT JOIN employees    e ON e.id = a.employee_id
           LEFT JOIN client_users c ON c.id = a.client_id
          WHERE ${f.join(" AND ")}`,g),y=Number(x[0]?.n||0),z=await (0,i.P)("SELECT status, COUNT(*) AS n FROM assets WHERE holder = ? GROUP BY status",[d]),A={};for(let a of o.aN)A[a]=0;let B=0;for(let a of z)A[a.status]=Number(a.n),B+=Number(a.n);let C=o.jS.map(()=>"?").join(", "),D=await (0,i.P)(`SELECT COALESCE(SUM(purchase_cost), 0) AS cost, COUNT(*) AS n
           FROM assets
          WHERE holder = ? AND ownership = 'ansar'
            AND status NOT IN (${C}) AND purchase_cost IS NOT NULL`,[d,...o.jS]),E=await (0,i.P)(`SELECT COUNT(*) AS n FROM assets
          WHERE holder = ? AND ownership = 'client' AND status NOT IN (${C})`,[d,...o.jS]),F=await (0,i.P)(`SELECT
         ...
         FROM (
           SELECT a.id,
             ...
             FROM assets a
            WHERE a.holder = ? AND a.status NOT IN (${C})
         ) c`,[d,...o.jS]),G=await (0,i.P)(`SELECT COUNT(DISTINCT a.id) AS n
           FROM assets a
           JOIN asset_maintenance m ON m.asset_id = a.id
          WHERE a.holder = ? AND m.next_due IS NOT NULL AND m.next_due < CURDATE()`,[d]),H=await (0,i.P)(`SELECT COUNT(*) AS n FROM assets
          WHERE holder = ? AND status NOT IN (${C})
            AND (serial_no IS NULL OR serial_no = '')
            AND (tag_no IS NULL OR tag_no = '')
            AND label_printed_at IS NULL`,[d,...o.jS]);return b.status(200).json({success:!0,data:w,page:t,per_page:r,total:y,summary:{...A,total:B,in_register:Number(D[0]?.n||0),total_cost:Number(D[0]?.cost||0),client_owned:Number(E[0]?.n||0),cover_expired:Number(F[0]?.expired||0),cover_expiring:Number(F[0]?.expiring||0),uncovered:Number(F[0]?.uncovered||0),service_overdue:Number(G[0]?.n||0),needs_label:Number(H[0]?.n||0)}})}if("POST"===a.method){if(!(0,j.OD)(c,b,e,"create"))return;let f=(0,j.J9)(a),g=(0,l.gx)(f.name,200);if(!g)return b.status(400).json({success:!1,error:"An asset name is required."});let h=(0,l.yL)(f.status,o.aN,"in_store");if(o.W9.includes(h)&&!(0,j.OD)(c,b,e,"dispose"))return;let n="ansar";if("external"===d){let a=String(f.ownership||"").trim();if(!o.v9.includes(a))return b.status(400).json({success:!1,error:"Ownership is required on an installed asset: `ansar` if ANSAR still owns it (rented, or not yet handed over), `client` if it was sold to the client."});n=a}let p="internal"===d?(0,l.fk)(f.employee_id):null,q="external"===d?(0,l.fk)(f.client_id):null;if((p||q)&&!(0,j.OD)(c,b,e,"assign"))return;let t="external"===d?(0,l.fk)(f.site_id):null;if(t){let a=await (0,i.P)(`SELECT s.id, s.name, s.client_id, s.is_active, c.company_name
             FROM asset_sites s
