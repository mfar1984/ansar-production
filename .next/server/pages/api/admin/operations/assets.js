"use strict";(()=>{var a={};a.id=4479,a.ids=[4479],a.modules={3498:a=>{a.exports=require("mysql2/promise")},75600:a=>{a.exports=require("next/dist/compiled/next-server/pages-api.runtime.prod.js")},99558:(a,b,c)=>{c.r(b),c.d(b,{config:()=>v,default:()=>u,handler:()=>x});var d={};c.r(d),c.d(d,{default:()=>p});var e=c(29046),f=c(8667),g=c(33480),h=c(86435),i=c(88251),j=c(3557),k=c(19275),l=c(96543),m=c(92102),n=c(95514);let o={internal:"assets_internal",external:"assets_external"};async function p(a,b){let c=await (0,j.OC)(a,b);if(!c)return;b.setHeader("Cache-Control","no-store");let d=String(a.query.holder||"").trim();if(!n.F7.includes(d))return b.status(400).json({success:!1,error:"A register must be named: holder=internal or holder=external."});let e=o[d];try{if("GET"===a.method){if(!(0,j.OD)(c,b,e,"view"))return;if("1"===String(a.query.options||"")){let[a,c,d,e,f,g,h,j,k]=await Promise.all([(0,i.P)(`SELECT id, name, icon, useful_life_years, default_loanable FROM asset_categories
              ...
              WHERE is_active = 1 ORDER BY name ASC`),(0,i.P)(`SELECT id, brand, model_name, category,
               ...
               FROM asset_models
              WHERE is_active = 1 ORDER BY brand ASC, model_name ASC`)]);return b.status(200).json({success:!0,categories:a,employees:c,departments:d,clients:e,projects:f,vendors:g,tenders:h,sites:j,models:k})}let f=["a.holder = ?"],g=[d],h=String(a.query.status||"").trim();if(h&&"all"!==h){if(!n.aN.includes(h))return b.status(400).json({success:!1,error:"Unknown status filter."});f.push("a.status = ?"),g.push(h)}let k=(0,l.gx)(a.query.category,80);k&&(f.push("a.category = ?"),g.push(k));let m="1"===String(a.query.all||"");if(m&&!(0,j.OD)(c,b,e,"export"))return;let o=Number(a.query.per_page),p=m?5e3:Number.isFinite(o)?Math.min(100,Math.max(1,Math.trunc(o))):100,q=Number(a.query.page),r=m||!Number.isFinite(q)?1:Math.max(1,Math.trunc(q)),s=(r-1)*p,t=String(a.query.q||"").trim();if(t){let a=(0,l.AR)(t);f.push("(a.asset_no LIKE ? ESCAPE '\\\\' OR a.name LIKE ? ESCAPE '\\\\' OR a.serial_no LIKE ? ESCAPE '\\\\' OR a.tag_no LIKE ? ESCAPE '\\\\' OR a.brand LIKE ? ESCAPE '\\\\' OR a.model LIKE ? ESCAPE '\\\\' OR e.full_name LIKE ? ESCAPE '\\\\' OR c.company_name LIKE ? ESCAPE '\\\\')"),g.push(a,a,a,a,a,a,a,a)}let u=await (0,i.P)(`SELECT a.id, a.asset_no, a.holder, a.name, a.category, a.brand, a.model,
                ...
                -- already covered.
                --
                -- The union brings in the DLP INHERITED from the tender, which is not a row in
                -- asset_obligations at all: it belongs to the contract, and every unit delivered
                -- under that award shares it. The NOT EXISTS is the override rule -- an asset
                -- carrying its own dlp obligation ignores the inherited one, which matters when
                -- the override SHORTENS it and a plain MAX would pick the longer date.
                ...
                DATE_FORMAT((SELECT MAX(x.ends_on) FROM (
                   SELECT o.ends_on
          ...
          WHERE ${f.join(" AND ")}
          ORDER BY a.id DESC
          LIMIT ${p} OFFSET ${s}`,g),v=await (0,i.P)(`SELECT COUNT(*) AS n
           FROM assets a
           LEFT JOIN employees    e ON e.id = a.employee_id
           LEFT JOIN client_users c ON c.id = a.client_id
          WHERE ${f.join(" AND ")}`,g),w=Number(v[0]?.n||0),x=await (0,i.P)("SELECT status, COUNT(*) AS n FROM assets WHERE holder = ? GROUP BY status",[d]),y={};for(let a of n.aN)y[a]=0;let z=0;for(let a of x)y[a.status]=Number(a.n),z+=Number(a.n);let A=n.jS.map(()=>"?").join(", "),B=await (0,i.P)(`SELECT COALESCE(SUM(purchase_cost), 0) AS cost, COUNT(*) AS n
           FROM assets
          WHERE holder = ? AND ownership = 'ansar'
            AND status NOT IN (${A}) AND purchase_cost IS NOT NULL`,[d,...n.jS]),C=await (0,i.P)(`SELECT COUNT(*) AS n FROM assets
          WHERE holder = ? AND ownership = 'client' AND status NOT IN (${A})`,[d,...n.jS]),D=await (0,i.P)(`SELECT
                  ...
                  (SELECT MAX(x.ends_on) FROM (
                     ...
                     SELECT DATE_ADD(t2.handover_date, INTERVAL t2.dlp_months MONTH)
                      ...
                      WHERE t2.id = a.tender_id
                        ...
                        AND NOT EXISTS (SELECT 1 FROM asset_obligations o2
                                         WHERE o2.asset_id = a.id AND o2.kind = 'dlp')
                   ) x)                                                             AS cover_until
             FROM assets a
            WHERE a.holder = ? AND a.status NOT IN (${A})
         ) c`,[d,...n.jS]),E=await (0,i.P)(`SELECT COUNT(DISTINCT a.id) AS n
           FROM assets a
           JOIN asset_maintenance m ON m.asset_id = a.id
          WHERE a.holder = ? AND m.next_due IS NOT NULL AND m.next_due < CURDATE()`,[d]);return b.status(200).json({success:!0,data:u,page:r,per_page:p,total:w,summary:{...y,total:z,in_register:Number(B[0]?.n||0),total_cost:Number(B[0]?.cost||0),client_owned:Number(C[0]?.n||0),cover_expired:Number(D[0]?.expired||0),cover_expiring:Number(D[0]?.expiring||0),uncovered:Number(D[0]?.uncovered||0),service_overdue:Number(E[0]?.n||0)}})}if("POST"===a.method){if(!(0,j.OD)(c,b,e,"create"))return;let f=(0,j.J9)(a),g=(0,l.gx)(f.name,200);if(!g)return b.status(400).json({success:!1,error:"An asset name is required."});let h=(0,l.yL)(f.status,n.aN,"in_store");if(n.W9.includes(h)&&!(0,j.OD)(c,b,e,"dispose"))return;let o="ansar";if("external"===d){let a=String(f.ownership||"").trim();if(!n.v9.includes(a))return b.status(400).json({success:!1,error:"Ownership is required on an installed asset: `ansar` if ANSAR still owns it (rented, or not yet handed over), `client` if it was sold to the client."});o=a}let p="internal"===d?(0,l.fk)(f.employee_id):null,s="external"===d?(0,l.fk)(f.client_id):null;if((p||s)&&!(0,j.OD)(c,b,e,"assign"))return;let t="external"===d?(0,l.fk)(f.site_id):null;if(t){let a=await (0,i.P)(`SELECT s.id, s.name, s.client_id, s.is_active, c.company_name
             FROM asset_sites s
             JOIN client_users c ON c.id = s.client_id
            WHERE s.id = ? LIMIT 1`,[t]);if(0===a.length)return b.status(400).json({success:!1,error:"That site no longer exists."});let c=a[0];if(!s||Number(c.client_id)!==s)return b.status(400).json({success:!1,error:`"${c.name}" belongs to ${c.company_name}. Choose a site belonging to this asset's client, or set the client first.`});if(1!==c.is_active)return b.status(400).json({success:!1,error:`"${c.name}" is inactive and cannot take new equipment. Reactivate it, or choose another site.`})}let u=(0,l.fk)(f.model_id),v=null;if(u){let a=await (0,i.P)(`SELECT brand, model_name, category, default_useful_life_years, default_warranty_months
             FROM asset_models WHERE id = ? LIMIT 1`,[u]);if(0===a.length)return b.status(400).json({success:!1,error:"That model no longer exists."});v=a[0]}for(let[a,c,e]of[[p,"employees","That employee no longer exists."],["internal"===d?(0,l.fk)(f.department_id):null,"departments","That department no longer exists."],[s,"client_users","That client no longer exists."],["external"===d?(0,l.fk)(f.project_id):null,"projects","That project no longer exists."],[(0,l.fk)(f.vendor_id),"vendors","That vendor no longer exists."],["external"===d?(0,l.fk)(f.tender_id):null,"tenders","That tender no longer exists."]]){if(!a)continue;let d=await (0,i.P)(`SELECT id FROM \`${c}\` WHERE id = ? LIMIT 1`,[a]);if(0===d.length)return b.status(400).json({success:!1,error:e})}let w=(0,l.gx)(f.category,80)||v?.category||null,x=0;if("internal"===d){if(void 0!==f.is_loanable)x=+!!f.is_loanable;else if(w){let a=await (0,i.P)("SELECT default_loanable FROM asset_categories WHERE name = ? LIMIT 1",[w]);x=+(1===Number(a[0]?.default_loanable))}}let y={name:g,category:w,brand:(0,l.gx)(f.brand,120)||v?.brand||null,model:(0,l.gx)(f.model,120)||v?.model_name||null,serial_no:(0,l.gx)(f.serial_no,120),tag_no:(0,l.gx)(f.tag_no,60),description:(0,l.gx)(f.description,65535),purchase_date:(0,l._$)(f.purchase_date),purchase_cost:(0,l.TG)(f.purchase_cost),useful_life_years:q(f.useful_life_years)??q(v?.default_useful_life_years),vendor_id:(0,l.fk)(f.vendor_id),tender_id:"external"===d?(0,l.fk)(f.tender_id):null,invoice_no:(0,l.gx)(f.invoice_no,80),employee_id:p,department_id:"internal"===d?(0,l.fk)(f.department_id):null,location:"internal"===d?(0,l.gx)(f.location,200):null,client_id:s,project_id:"external"===d?(0,l.fk)(f.project_id):null,site_id:t,site_name:"external"===d?(0,l.gx)(f.site_name,200):null,site_address:"external"===d?(0,l.gx)(f.site_address,400):null,installed_on:"external"===d?(0,l._$)(f.installed_on):null,status:h,condition_note:(0,l.gx)(f.condition_note,400),notes:(0,l.gx)(f.notes,65535)},z=0,A="";for(let a=0;a<5;a++){A=await (0,m.fp)("assets","asset_no","internal"===d?"AST":"EXA");try{z=(await (0,i.P)(`INSERT INTO assets
